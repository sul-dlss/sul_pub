...
namespace :cap_cutover do
  ...
  task :pull_pubmed_for_cap, [:file_location] => :environment do |_t, args|
    ...
    cap_import_pmid_logger.info 'Started pumed import ' + Time.zone.now.to_s
    lines = CSV.foreach(args.file_location, headers: true, header_converters: :symbol) do |row|
      ...
      GC.start if total_running_count % 4000 == 0
    end
    # finish off the batch
    PubmedSourceRecord.get_and_store_records_from_pubmed(pmids_for_this_batch)
    new_downloads += pmids_for_this_batch.size
    Rails.logger.info total_running_count.to_s + 'records were processed in ' + distance_of_time_in_words_to_now(start_time, true)
    Rails.logger.info lines.to_s + ' lines of file: ' + args.file_location.to_s + ' were processed.'
    Rails.logger.info new_downloads.to_s + 'new records downloaded'
    cap_import_pmid_logger.info 'Finished pubmed import.' + Time.zone.now.to_s
    cap_import_pmid_logger.info lines.to_s + ' lines of file: ' + args.file_location.to_s + ' were processed.'
    cap_import_pmid_logger.info total_running_count.to_s + 'records were processed in ' + distance_of_time_in_words_to_now(start_time, true)
  ...
  task :pull_sw_for_cap, [:file_location] => :environment do |_t, args|
    include ActionView::Helpers::DateHelper
    start_time = Time.zone.now
    total_running_count = 0
    new_downloads = 0
    ...
    cap_import_sw_logger.info 'Started sciencewire import ' + Time.zone.now.to_s
    lines = CSV.foreach(args.file_location, headers: true, header_converters: :symbol) do |row|
      total_running_count += 1
      pmid = row[:pubmed_id].to_s
      ...
      GC.start if total_running_count % 5000 == 0
    end
    SciencewireSourceRecord.get_and_store_sw_source_records(pmids_for_this_batch)
    Rails.logger.info "#{total_running_count} in #{distance_of_time_in_words_to_now(start_time)}"
    Rails.logger.info lines.to_s + ' lines of file: ' + args.file_location.to_s + ' were processed.'
    Rails.logger.info new_downloads.to_s + 'new records downloaded'
    cap_import_sw_logger.info 'Finished sciencewire import.' + Time.zone.now.to_s
    cap_import_sw_logger.info lines.to_s + ' lines of file: ' + args.file_location.to_s + ' were processed.'
    cap_import_sw_logger.info total_running_count.to_s + 'records were processed in ' + distance_of_time_in_words_to_now(start_time, true)
  ...
  task :create_pubs_from_cap, [:file_location] => :environment do |_t, args|
    include ActionView::Helpers::DateHelper
    start_time = Time.zone.now
    total_running_count = 0
    @cap_import_logger = Logger.new(Rails.root.join('log', 'cap_create_pmid_pubs.log'))
    @cap_import_logger.info 'Started cap build pub process ' + Time.zone.now.to_s
    ...
    lines = CSV.foreach(args.file_location, headers: true, header_converters: :symbol) do |row|
      ...
      if total_running_count % 1000 == 0
        Rails.logger.debug "#{total_running_count} in #{distance_of_time_in_words_to_now(start_time, true)}"
        @cap_import_logger.info total_running_count.to_s + ' in ' + distance_of_time_in_words_to_now(start_time, true)
      end
    end
    @cap_import_logger.info 'Finished import.' + Time.zone.now.to_s
    @cap_import_logger.info lines.to_s + ' lines of file: ' + args.file_location.to_s + ' were processed.'
    @cap_import_logger.info total_running_count.to_s + 'records were processed in ' + distance_of_time_in_words_to_now(start_time, true)
  end

  desc 'create publication and contribution records from full cap delta'
  task :create_pubs_from_delta, [:file_location] => :environment do |_t, args|
    include ActionView::Helpers::DateHelper
    start_time = Time.zone.now
    total_running_count = 0
    @cap_import_logger = Logger.new(Rails.root.join('log', 'cap_create_pmid_pubs.log'))
  ...
  task :ingest_man_pubs, [:file_location] => :environment do |_t, args|
    include ActionView::Helpers::DateHelper
    start_time = Time.zone.now
    total_running_count = 0
    @cap_manual_import_logger = Logger.new(Rails.root.join('log', 'cap_import_man_pubs.log'))
    @cap_manual_import_logger.info 'Started cap manual pub import process ' + Time.zone.now.to_s
    lines = CSV.foreach(args.file_location, headers: true, header_converters: :symbol) do |row|
      total_running_count += 1
      Rails.logger.debug "#{total_running_count} in #{distance_of_time_in_words_to_now(start_time, true)}" if total_running_count % 500 == 0
      author = Author.where(cap_profile_id: row[:profile_id]).first
    ...
    @cap_manual_import_logger.info total_running_count.to_s + 'records were processed in ' + distance_of_time_in_words_to_now(start_time, true)
  end

  desc 'create or update manual publications from delta'
  task :man_pubs_from_delta, [:file_location] => :environment do |_t, args|
    include ActionView::Helpers::DateHelper
    start_time = Time.zone.now
    total_running_count = 0
    @cap_manual_import_logger = Logger.new(Rails.root.join('log', 'cap_import_man_pubs.log'))
      ...
      begin
        ...
        if (result.count == 1)
          ...
          if man_record.nil?
            ...
            pub.update_manual_pub_from_pub_hash(pub_hash, original_source)
          end
        else
  ...
  task build_pubs: :environment do
    ...
    @cap_import_logger.info 'Finished build.' + Time.zone.now.to_s
    @cap_import_logger.info total_running_count.to_s + 'records were processed in ' + distance_of_time_in_words_to_now(start_time, true)
  end
  ...
  task build_missing_pubs: :environment do
    ...
    @cap_import_logger.formatter = proc { |severity, datetime, _progname, msg|
  ...
  desc 'utility to rebuild pub hashes from sciencewire and pubmed sources for all pubs'
  task :build_delta_pubs, [:file_location] => :environment do |_t, args|
    include ActionView::Helpers::DateHelper
    start_time = Time.zone.now
    total_running_count = 0
  ...
  task rewrite_authorship: :environment do
    ...
    @rebuild_authorship_logger.info 'Started resync process ' + Time.zone.now.to_s
    ...
    Publication.find_each do |pub|
      ...
      if total_running_count % 1000 == 0
        Rails.logger.debug "#{total_running_count} in #{distance_of_time_in_words_to_now(start_time, true)}"
        @rebuild_authorship_logger.info total_running_count.to_s + ' in ' + distance_of_time_in_words_to_now(start_time, true)
      end
    end
    @rebuild_authorship_logger.info 'Finished build.' + Time.zone.now.to_s
    @rebuild_authorship_logger.info total_running_count.to_s + 'records were processed in ' + distance_of_time_in_words_to_now(start_time, true)
  end
  ...
  task update_pubs: :environment do
    ...
    @cap_import_logger.info 'Started cap update pub process ' + Time.zone.now.to_s
    ...
    @cap_import_logger.info 'Finished update.' + Time.zone.now.to_s
    @cap_import_logger.info total_running_count.to_s + 'records were updated in ' + distance_of_time_in_words_to_now(start_time, true)
  end
  ...
  def build_pub_from_sw_and_pubmed(pub)
    ...
    if sciencewire_source_record.present?
      sw_pub_hash = sciencewire_source_record.source_as_hash
      ...
      pubmed_hash = pubmed_source_record.source_as_hash
      pub.update_attributes(
        active: true,
  ...
  task :build_missing_pubs_one_item, [:pub_id] => :environment do |_t, args|
    @cap_import_logger = Logger.new(Rails.root.join('log', 'cap_build_pubs.log'))
    @cap_import_logger.datetime_format = '%Y-%m-%d %H:%M:%S'
    @cap_import_logger.formatter = proc { |severity, datetime, _progname, msg|
      "#{severity} #{datetime}: #{msg}\n"
  ...
  def build_pub_from_missing_sw_and_pubmed(pub)
    ...
    if pmid.blank? && sciencewire_id.blank?
