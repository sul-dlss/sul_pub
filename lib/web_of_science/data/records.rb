...
module WebOfScience
  ...
  module Data
    ...
    class Records
      ...
      # @param xmlrecords [String] records in XML
      # @param encoded_records [String] records in HTML encoding
      def initialize(xmlrecords: nil, encoded_records: nil)
        @doc = WebOfScience::XmlParser.parse(xmlrecords, encoded_records)
      end

      # Group records by the database prefix in the UID
      #  - where a database prefix is missing, groups records into 'MISSING_DB'
      # @return [Hash<String => WebOfScience::Data>]
      def by_database
        ...
        db_recs.each_key do |db|
          ...
          db_recs[db] = WebOfScience::Data::Records.new(records: rec_doc.to_xml)
        end
        db_recs
      end

      # Iterate over WebOfScience::WokRecord objects
      # @yield records [WebOfScience::Record]
      def each
        rec_nodes.each { |rec| yield WebOfScience::Data::Record.new(record: rec.to_xml) }
      end
      ...
      # @param record_setB [WebOfScience::Data]
      ...
      # Merge WoS records
      # @param record_setB [WebOfScience::Data]
      # @return records [WebOfScience::Data] merged set of records
      def merge_records(record_setB)
        ...
        WebOfScience::Data::Records.new(records: docA.to_xml)
      end
