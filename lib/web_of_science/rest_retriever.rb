...
module WebOfScience
  ...
  class RestRetriever
    ...
    def initialize(params, path: '/', database: 'WOK', batch_size: MAX_RECORDS)
      @batch_iteration = 0
    ...
    attr_reader :batch_size, :query, :query_id, :path, :base_params, :database
    ...
    def batch_one
      @batch_one ||= begin
        response = client.xml_get(path, params)
        ...
        # @records_found = response.at('/WosSearchResponse/QueryResult/RecordsFound').to_i
        ...
        # @query_id = response.at('/WosSearchResponse/QueryResult/QueryID')
    ...
    def retrieve_batch
      @batch_iteration += 1
      response = client.xml_get("/query/#{query_id}", params)
