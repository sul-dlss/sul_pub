...
describe SciencewireSourceRecord, :vcr do
  describe '.lookup_sw_doc_type' do
    it 'maps document types to CAP inproceedings' do
      expect(described_class.lookup_sw_doc_type(['Meeting Abstract'])).to eq('inproceedings')
    end

    it 'maps document types to CAP books' do
      expect(described_class.lookup_sw_doc_type(['Government Publications'])).to eq('book')
    end

    it 'uses case insensitive string comparisons' do
      expect(described_class.lookup_sw_doc_type(['consensus Development Conference nih'])).to eq('inproceedings')
    end
    ...
    it 'defaults to article' do
      expect(described_class.lookup_sw_doc_type('Congresses')).to eq('inproceedings')
    end
  end
  ...
  describe '.lookup_cap_doc_type_by_sw_doc_category' do
    it "maps DocumentCategory='Conference Proceeding Document' to cap type of 'inproceedings'" do
      expect(described_class.lookup_cap_doc_type_by_sw_doc_category('Conference Proceeding Document')).to eq('inproceedings')
    end

    it "maps DocumentCategory='Journal Document' to cap type of 'article'" do
      expect(described_class.lookup_sw_doc_type('Journal Document')).to eq('article')
    end

    it 'defaults to article' do
      expect(described_class.lookup_sw_doc_type('Other')).to eq('article')
    end
  end
  ...
  describe 'instance methods' do
    ...
    it '#sciencewire_update raises error' do
      expect { subject.sciencewire_update }.to raise_error RuntimeError
    end
  end
  ...
  describe '.source_as_hash' do
    describe 'parses one publication' do
      ...
      it 'extracts identifiers' do
        ...
        expect(subject[:identifier][3]).to include(type: 'doi', id: '10.1007/s13280-013-0447-x', url: 'https://doi.org/10.1007/s13280-013-0447-x')
      end
    end
