...
describe 'cleanup rake tasks' do
  ...
  describe 'merge profiles task' do
    let(:primary_author) do
      ...
      allow(auth).to receive_messages(publications: [build(:publication, id: 1), build(:publication, id: 2)],
                                      contributions: [Contribution.new(id: 1, publication_id: 1),
    ...
    let(:duped_author) do
      ...
      allow(auth).to receive_messages(publications: [primary_author.publications.last.dup,
                                                     build(:publication, id: 3)], contributions: [Contribution.new(publication_id: 2, author_id: 2),
