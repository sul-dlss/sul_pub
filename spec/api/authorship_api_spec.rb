...
describe SulBib::API do
  ...
  let(:valid_json_for_sw_id) { { sul_author_id: author.id, status: 'denied', visibility: 'private', sw_id: 10_379_039 }.to_json }
  let(:valid_json_for_pubmed_id) { { sul_author_id: author.id, status: 'denied', visibility: 'private', pmid: 23_684_686 }.to_json }
  ...
  describe 'POST /authorship' do
    context 'when post is valid ' do
      ...
      it 'increases number of contribution records, where records already exist, by one' do
        ...
        end.to change { publication_with_contributions.contributions(true).length }.by(1)
      end
      ...
      it 'should update status of an existing contribution' do
        expect do
          post '/authorship', valid_json_for_pub_with_contributions, headers
        end.to change { existing_contrib.reload.status }
      end
      it 'should update visibility of an existing contribution' do
        expect do
          post '/authorship', valid_json_for_pub_with_contributions, headers
        end.to change { existing_contrib.reload.visibility }
      end
