describe WebOfScience::Services::HashMappers::NamesMapper do
  ...
  context 'WOS records' do
    ...
    let(:pub_hash) { mapper.map_names_to_hash(wos_record) }
  ...
  context 'MEDLINE records' do
    subject(:mapper) { described_class.new }
    let(:pub_hash) { mapper.map_names_to_hash(medline_record) }
