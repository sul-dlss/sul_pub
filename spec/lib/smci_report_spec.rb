describe SMCIReport do
  ...
  describe '#output_row' do
    ...
    it 'creates an output for a profile author' do
      result = report.send(:output_row, pub_hash, author, date, 'new')
    ...
    it 'creates an output row for a non-profile author' do
      result = report.send(:output_row, pub_hash, nil, date, 'unknown')
      expect(result).to eq(['some title', '', '', '', '', '', '', 'some publisher', '', '', '', '', nil, nil, '', nil, nil, nil, '', '', '', '', '', '', 'unknown', date, nil, nil, nil])
    end
  end
end
